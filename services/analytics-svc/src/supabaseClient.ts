...
/**
 * Same trust model as the monolith: only the anon key is used here, never
 * service_role. RLS + RPC grants on the `anon`/`authenticated` roles are what
 * actually gate access — this service does not elevate privileges.
 */
