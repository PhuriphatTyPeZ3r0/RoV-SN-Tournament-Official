...
async function run() {
    ...
    try {
        ...
        const existing = await sql`SELECT * FROM public.tournaments WHERE season = 2027`;
        if (existing.length > 0) {
            console.log("Season 2027 already exists!");
            console.log(existing);
            return;
        }

        const inserted = await sql`
            INSERT INTO public.tournaments (name, season, status)
            VALUES ('RoV SN Tournament 2027', 2027, 'completed')
            RETURNING *
        `;
        console.log("Inserted mock season:");
