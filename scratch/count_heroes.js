...
async function run() {
    ...
    try {
        const result = await sql`SELECT count(*) FROM public.heroes`;
        console.log("Number of heroes in DB:", result[0].count);

        const sample = await sql`SELECT * FROM public.heroes LIMIT 5`;
        console.log("Sample heroes:", sample);
    } catch (err) {
