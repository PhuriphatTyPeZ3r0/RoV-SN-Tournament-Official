...
export default function AdminRegistrationsPage() {
    ...
    const handleAction = async (id: string, status: 'approved' | 'rejected') => {
        const { value: notes } = await Swal.fire({
            title: status === 'approved' ? t.admin.registrationsPage.confirmApproveTitle : t.admin.registrationsPage.confirmRejectTitle,
            input: 'text',
            inputLabel: t.admin.registrationsPage.notesLabel,
            inputPlaceholder: t.admin.registrationsPage.notesPlaceholder,
            showCancelButton: true,
            confirmButtonText: status === 'approved' ? t.admin.registrationsPage.approveBtn : t.admin.registrationsPage.rejectBtn,
        ...
        if (notes !== undefined) {
            ...
            if (result.success) {
                Swal.fire(language === 'th' ? 'สำเร็จ' : 'Success', t.admin.registrationsPage.successMsg, 'success');
                fetchRegistrations();
            } else {
                Swal.fire(language === 'th' ? 'ข้อผิดพลาด' : 'Error', result.error || t.admin.registrationsPage.errorMsg, 'error');
            }
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">{t.admin.registrationsPage.subtitle}</p>
                </div>
