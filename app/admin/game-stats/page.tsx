    ...
    const filteredStats = playerStats
        ...
        .sort((a, b) => {
            switch (sortBy) {
                ...
                case 'mvp': return (b.mvpCount || 0) - (a.mvpCount || 0);
                case 'winRate': return (b.winRate || 0) - (a.winRate || 0);
                case 'kda':
                default: return (b.kda || 0) - (a.kda || 0);
            }
        });
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase">{t.admin.dashboard.manageTeamsDesc}</th>
                    ...
                    <div key={`${player.teamName}-${player.realName}`} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm relative">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            ...
                            <div className="bg-cyan-50 p-2 rounded text-center border border-cyan-100">
                                ...
                                <div className="font-bold text-cyan-700">{(player.kda || 0).toFixed(2)}</div>
                            </div>
                        </div>
            ...
            {playerStats.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white">
                        <div className="text-3xl font-bold">{playerStats.reduce((acc, p) => acc + (p.totalKills || 0), 0)}</div>
                        <div className="text-blue-100 text-sm">{t.stats.kills}</div>
                    </div>
                    <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-5 text-white">
                        <div className="text-3xl font-bold">{playerStats.reduce((acc, p) => acc + (p.totalAssists || 0), 0)}</div>
                        <div className="text-green-100 text-sm">{t.stats.assists}</div>
                    </div>
                    <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-5 text-white">
                        <div className="text-3xl font-bold">{playerStats.reduce((acc, p) => acc + (p.mvpCount || 0), 0)}</div>
                        <div className="text-yellow-100 text-sm">Total MVP</div>
                    </div>
                    <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-xl p-5 text-white">
                        <div className="text-3xl font-bold">
                            {(playerStats.reduce((acc, p) => acc + (p.kda || 0), 0) / playerStats.length).toFixed(2)}
                        </div>
