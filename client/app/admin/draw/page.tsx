...
export default function AdminDrawPage() {
    ...
    const handleGenerateDraw = () => {
        ...
        Swal.fire({
            title: 'ยืนยันการสุ่มจับคู่?',
            text: `ระบบจะสุ่มทีมที่พร้อมแข่งขันมาจับคู่กันใน Day ${matchDay}`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'เริ่มสุ่มจับคู่',
            ...
            if (result.isConfirmed) {
                startTransition(async () => {
                    try {
                        ...
                        if (res.success) {
                            Swal.fire('สำเร็จ', `สร้างการแข่งขัน ${res.count} คู่เรียบร้อย`, 'success');
                            fetchMatches();
                        }
                    } catch (error: any) {
    ...
    const handleClearDraw = () => {
        Swal.fire({
            ...
            text: `การแข่งขันทั้งหมดของ Day ${matchDay} จะถูกลบออก`,
            icon: 'warning',
            showCancelButton: true,
        ...
        <div className="space-y-8 animate-fadeIn">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">สุ่มจับคู่การแข่งขันอัตโนมัติ</p>
                </div>
                ...
                <div className="lg:col-span-1 space-y-4">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                            ...
                            {readyTeams.length === 0 && (
                                <p className="text-center text-gray-400 text-sm py-4">ไม่พบทีมที่มีสถานะ Ready</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            {matches.map((match, idx) => (
                                <div key={match.id} className="flex items-center bg-gray-50 rounded-2xl p-4 border border-gray-100 relative group overflow-hidden">
                                    ...
                                    <div className="px-8 flex flex-col items-center">
                                        ...
                                        <div className="text-[10px] font-bold text-gray-400 mt-1 uppercase">Match {idx + 1}</div>
                                    </div>
                            ...
                            {matches.length === 0 && (
                                <div className="flex flex-col items-center justify-center py-20 text-gray-300">
                                    ...
                                    <p className="text-gray-400">ยังไม่มีการจัดสายการแข่งขันในวันนี้</p>
                                </div>
                            )}
                        </div>
                    </div>
