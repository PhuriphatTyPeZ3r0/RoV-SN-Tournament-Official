...
async function getPlayerGamingProfile() {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return null;
    let { data: player } = await supabase.from('players').select('id, in_game_name, open_id, current_rank, lineup_role, secondary_role, top_heroes, experience_bio, favorite_heroes, nickname, phone').eq('profile_id', user.id).maybeSingle();
    // Self-healing: if profile is verified but player row is missing, auto-create it
    if (!player) {
        const { data: profile } = await supabase.from('profiles').select('registration_status, full_name, class_grade, in_game_name').eq('id', user.id).maybeSingle();
        if (profile && (profile.registration_status === 'verified' || profile.registration_status === 'approved')) {
            const { data: newPlayer, error: createError } = await supabase.from('players').insert({
                profile_id: user.id,
                name: profile.full_name || 'Anonymous Player',
                grade: profile.class_grade || '',
                in_game_name: profile.in_game_name || ''
            }).select('id, in_game_name, open_id, current_rank, lineup_role, secondary_role, top_heroes, experience_bio, favorite_heroes, nickname, phone').single();
...
//# sourceMappingURL=_67107319._.js.map