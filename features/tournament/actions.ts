...
export async function updateTournamentThemeAction(
  tournamentId: string,
  themeStyle: string
) {
  const { supabase } = await requireAdmin();
...
const colorHexSchema = z.string().regex(/^#[0-9A-F]{6}$/i, 'รหัสสีต้องเป็น Hex Code ขึ้นต้นด้วย # ตามด้วยเลขฐานสิบหก 6 หลัก (เช่น #15C8FF)');

const themeValidationSchema = z.object({
  id: z.string().min(1, 'ไอดีธีมต้องไม่ว่างเปล่า').regex(/^[a-z0-9-]+$/, 'ไอดีต้องเป็นภาษาอังกฤษพิมพ์เล็ก ตัวเลข หรือเครื่องหมาย - เท่านั้น'),
...
export async function createThemeAction(themeData: z.infer<typeof themeValidationSchema>) {
  const { supabase } = await requireAdmin();
...
export async function updateThemeAction(
  themeId: string,
  themeData: Omit<z.infer<typeof themeValidationSchema>, 'id'>
) {
  const { supabase } = await requireAdmin();
