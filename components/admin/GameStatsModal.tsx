...
export default function GameStatsModal({
    ...
    const handleAiUpload = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            Swal.fire(t.admin.gameStatsModal.successTitle, t.admin.gameStatsModal.aiSuccessText.replace('{count}', aiData.length.toString()), 'success');

        } catch (error: any) {
            console.error('AI Error:', error);
            Swal.fire(t.admin.gameStatsModal.errorTitle, t.admin.gameStatsModal.aiErrorText.replace('{error}', error.message || 'Unknown error'), 'error');
        } finally {
            setUploading(false);
            // Reset file input
            e.target.value = '';
            ...
            <div className="bg-white rounded-xl w-full max-w-7xl p-6 shadow-2xl m-4 animate-fade-in relative">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 flex flex-wrap gap-6 items-center">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="font-bold text-gray-700">{t.admin.gameStatsModal.winnerTeam}:</span>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="text-gray-500 border-b border-blue-200">
                                        <th className="p-2 text-center w-12">{t.stats.heroes}</th>
                                        <th className="p-2 text-left w-32">{t.admin.gameStatsModal.playerName}</th>
